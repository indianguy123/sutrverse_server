...
router.get('/stylists/:id/availability', async (req, res) => {
    try {
        ...
        const hours = stylist.salon.operatingHours.find((h) => h.dayOfWeek === dayOfWeek);
        if (!hours || hours.isClosed) {
            res.json({ data: { slots: [], closed: true }, error: null });
            return;
        }
        // Get booked appointments for that day
        const startOfDay = dayjs.tz(`${date} 00:00`, 'YYYY-MM-DD HH:mm', stylist.salon.timezone).utc().toDate();
        const endOfDay = dayjs.tz(`${date} 23:59:59`, 'YYYY-MM-DD HH:mm:ss', stylist.salon.timezone).utc().toDate();
        ...
        const slots = computeSlots(date, stylist.salon.timezone, hours.openTime, hours.closeTime, stylist.slotDuration, bookedAppointments);
        res.json({
            data: {
